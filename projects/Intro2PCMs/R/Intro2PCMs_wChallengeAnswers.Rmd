...
```{r}
td3 <- mutate(td, lifespan = log(X17.1_MaxLongevity_m), lnMass = log(X5.1_AdultBodyMass_g), precip=log(X28.1_Precip_Mean_mm)) %>% filter(., !is.na(lnMass), !is.na(lifespan), !is.na(precip)) %>% select(., lifespan, lnMass, precip)

lm_P <- phylolm(lifespan ~ precip, model="lambda", data=td3$dat, phy=td3$phy)
lm_PM <- phylolm(lifespan ~ precip + lnMass, model="lambda", data=td3$dat, phy=td3$phy)
lm_PxM <- phylolm(lifespan ~ precip*lnMass, model="lambda", data=td3$dat, phy=td3$phy)
lm_OLS <- lm(lifespan ~ precip*lnMass, data=td3$dat)
...
plot(td3[['precip']], td3[['lifespan']], cex=(td3[['lnMass']]-min(td3[['lnMass']]))/diff(range(td3[['lnMass']])), xlab="Precipitation", ylab="Max Longevity")
curve(lm_PxM$coefficients[1]+lm_PxM$coefficients[2]*x + lm_PxM$coefficients[3]*max(td3[['lnMass']]) + lm_PxM$coefficients[4]*x*max(td3[['lnMass']]), add=TRUE, lty=2)
curve(lm_PxM$coefficients[1]+lm_PxM$coefficients[2]*x + lm_PxM$coefficients[3]*min(td3[['lnMass']]) + lm_PxM$coefficients[4]*x*min(td3[['lnMass']]), add=TRUE, lty=3)
